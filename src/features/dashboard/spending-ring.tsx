...
export function SpendingRing({ transactions, categories, currency, groups }: { transactions: FinanceTransaction[]; categories: Category[]; currency: string; groups?: ReturnType<typeof spendingByCategory> }) {
  const values = (groups ?? spendingByCategory(transactions)).slice(0, 5);
  const data = values.map((item) => ({ id: item.categoryId, name: categories.find((category) => category.id === item.categoryId)?.name ?? "Other", value: Number(item.amountMinor) / 100, color: categories.find((category) => category.id === item.categoryId)?.color ?? financeColors.neutral }));
  const total = sumMoney(values.map((item) => item.amountMinor));
  return <div className="grid items-center gap-2 sm:grid-cols-[150px_minmax(0,1fr)]">
    ...
    <div className="space-y-2.5">{data.map((item) => <Link href={transactionHref({category:item.id,...dateRange("this_month")})} key={item.name} className="flex min-h-11 min-w-0 items-center gap-2 text-xs hover:text-info"><span className="size-2.5 rounded-full" style={{ background: item.color }} /><span className="min-w-0 flex-1 truncate text-muted">{item.name}</span><strong className="amount">{formatMoney(BigInt(Math.round(item.value * 100)), currency, "en-IN", true)}</strong></Link>)}</div>
  </div>;
}
