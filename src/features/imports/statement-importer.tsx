...
export function StatementImporter() {
  ...
  return <div className="space-y-5">
    ...
    {rows.length > 0 && <>
      ...
      <div className="grid gap-3 md:hidden">{visibleRows.map((row) => <MobileReviewRow key={row.id} row={row} accounts={activeAccounts} categories={activeCategories} purchases={data.transactions.filter((item) => item.type === "expense" && item.accountId === row.accountId)} onChange={(changes) => setRows((current) => updateRow(current, row.id, changes))} />)}</div>
...
function ReviewBadge({ row }: { row: NormalizedTransaction }) {
  ...
  return <span className={`inline-flex whitespace-nowrap rounded-full px-2 py-1 text-[10px] font-bold ${tone}`}>{label} · {row.confidence === "low" ? "low" : row.confidence}</span>;
}
