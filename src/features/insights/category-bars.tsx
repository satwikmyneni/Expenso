...
export function CategoryBars({ transactions, categories, currency }: { transactions: FinanceTransaction[]; categories: Category[]; currency: string }) {
  const data = spendingByCategory(transactions).slice(0, 7).map((item) => ({ name: categories.find((cat) => cat.id === item.categoryId)?.name.split(" ")[0] ?? "Other", amount: Number(item.amountMinor) / 100 }));
