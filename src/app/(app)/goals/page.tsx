...
export default function GoalsPage() {
  const { data, addGoal, updateGoal, archiveGoal, addGoalContribution, updateGoalContribution, deleteGoalContribution } = useFinance();
  const [creating, setCreating] = useState(false);
  const [editingGoal,setEditingGoal] = useState<Goal>();
  const [goalForm, setGoalForm] = useState({ name: "", target: "", targetDate: "", description: "", linkedAccountId: "", color: String(financeColors.accent), icon: "Target" });
  ...
  const createGoal = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const draft = { name: goalForm.name.trim(), targetMinor, currentMinor: editingGoal?.currentMinor ?? 0n, openingMinor: editingGoal?.openingMinor ?? 0n, targetDate: goalForm.targetDate, color: goalForm.color, icon: goalForm.icon, description: goalForm.description, linkedAccountId: goalForm.linkedAccountId || undefined };
      if (editingGoal) await updateGoal(editingGoal.id,draft); else await addGoal(draft);
      toast.success(editingGoal ? "Goal updated" : "Goal created");
      setGoalForm({ name: "", target: "", targetDate: "", description: "", linkedAccountId: "", color: String(financeColors.accent), icon: "Target" });
  ...
  return <>
    <PageHeading eyebrow="Make progress visible" title="Savings goals" description={`You’ve set aside ${formatMoney(totalCurrent, data.profile.currency)} across ${data.goals.length} priorities.`} action={<Button onClick={() => { setEditingGoal(undefined); setGoalForm({ name: "", target: "", targetDate: "", description: "", linkedAccountId: "", color: String(financeColors.accent), icon: "Target" }); setCreating(true); }}><Plus className="size-4" /> New goal</Button>} />
    ...
    <div className="grid gap-4 xl:grid-cols-2">{data.goals.map((goal) => <GoalCard key={goal.id} goal={goal} contributions={data.goalContributions.filter((item) => item.goalId === goal.id)} currency={data.profile.currency} accounts={data.accounts} onAdd={() => openContribution(goal)} onEdit={(contribution) => openContribution(goal, contribution)} onRemove={setRemovingContribution} onEditGoal={() => { setEditingGoal(goal); setGoalForm({name:goal.name,target:minorToDecimal(goal.targetMinor),targetDate:goal.targetDate,description:goal.description ?? "",linkedAccountId:goal.linkedAccountId ?? "",color:goal.color,icon:goal.icon}); setCreating(true); }} onArchive={() => { if(window.confirm("Archive this goal? Contributions and transaction history will be preserved.")) void archiveGoal(goal.id).then(()=>toast.success("Goal archived")).catch((error)=>toast.error(error.message)); }} />)}</div>
    ...
    <Modal open={creating} onClose={() => setCreating(false)} title={editingGoal ? "Edit savings goal" : "Create a savings goal"} description="New goals begin at zero; each amount saved is recorded in its contribution history."><form onSubmit={createGoal} className="grid gap-4 p-5 sm:p-6"><Field label="Goal name"><Input required value={goalForm.name} onChange={(event) => setGoalForm({ ...goalForm, name: event.target.value })} placeholder="e.g. Emergency fund" autoFocus /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Target amount"><Input required inputMode="decimal" value={goalForm.target} onChange={(event) => setGoalForm({ ...goalForm, target: event.target.value })} /></Field><Field label="Target date"><Input type="date" required value={goalForm.targetDate} onChange={(event) => setGoalForm({ ...goalForm, targetDate: event.target.value })} /></Field></div><Field label="Description"><Textarea value={goalForm.description} onChange={(event)=>setGoalForm({...goalForm,description:event.target.value})}/></Field><Field label="Linked account"><Select value={goalForm.linkedAccountId} onChange={(event)=>setGoalForm({...goalForm,linkedAccountId:event.target.value})}><option value="">No linked account</option>{sourceAccounts.map((account)=><option key={account.id} value={account.id}>{account.name}</option>)}</Select></Field><Field label="Goal color"><Input type="color" value={goalForm.color} onChange={(event)=>setGoalForm({...goalForm,color:event.target.value})}/></Field><Field label="Goal icon"><Select value={goalForm.icon} onChange={(event)=>setGoalForm({...goalForm,icon:event.target.value})}>{["Target","ShieldCheck","Plane","Laptop"].map((icon)=><option key={icon}>{icon}</option>)}</Select></Field><div className="flex justify-end gap-2 pt-2"><Button type="button" variant="ghost" onClick={() => setCreating(false)}>Cancel</Button><Button type="submit" disabled={busy}>{busy ? "Saving…" : editingGoal ? "Save goal" : "Create goal"}</Button></div></form></Modal>
...
function GoalCard({ goal, contributions, currency, accounts, onAdd, onEdit, onRemove, onEditGoal, onArchive }: { goal: Goal; contributions: GoalContribution[]; currency: string; accounts: { id: string; name: string }[]; onAdd: () => void; onEdit: (contribution: GoalContribution) => void; onRemove: (contribution: GoalContribution) => void; onEditGoal: () => void; onArchive: () => void }) {
