...
export default function GoalsPage() {
  const { data, addGoal, updateGoal, deleteGoal, addGoalContribution, updateGoalContribution, deleteGoalContribution } = useFinance();
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-2">{data.goals.map((goal) => <GoalCard key={goal.id} goal={goal} contributions={data.goalContributions.filter((item) => item.goalId === goal.id)} currency={data.profile.currency} accounts={data.accounts} onAdd={() => openContribution(goal)} onEdit={(contribution) => openContribution(goal, contribution)} onRemove={setRemovingContribution} onEditGoal={() => { setEditingGoal(goal); setGoalForm({name:goal.name,target:minorToDecimal(goal.targetMinor),targetDate:goal.targetDate,description:goal.description ?? "",linkedAccountId:goal.linkedAccountId ?? "",color:goal.color,icon:goal.icon}); setCreating(true); }} onDelete={() => { if(window.confirm("Delete this goal and its contributions? Financial transactions will be preserved.")) void deleteGoal(goal.id).then(()=>toast.success("Goal deleted")).catch((error)=>toast.error(error.message)); }} />)}</div>
...
function GoalCard({ goal, contributions, currency, accounts, onAdd, onEdit, onRemove, onEditGoal, onDelete }: { goal: Goal; contributions: GoalContribution[]; currency: string; accounts: { id: string; name: string }[]; onAdd: () => void; onEdit: (contribution: GoalContribution) => void; onRemove: (contribution: GoalContribution) => void; onEditGoal: () => void; onDelete: () => void }) {
